...
var warnOnBrokenImplementation = function(methodName, cause) {
  console.warn(`The browser extension that intercepted this WebAuthn API call incorrectly implemented ${methodName}. You should report this error to them.\n`, cause);
};
var bufferToUTF8String = function(value) {
  return new TextDecoder("utf-8").decode(value);
};
var browserSupportsWebAuthnAutofill = function() {
...
var identifyAuthenticationError = function({ error, options }) {
  const { publicKey } = options;
  if (!publicKey) {
    throw Error("options was missing required publicKey property");
  }
...
window.startPasskeyRegistration = () => {
  ...
  }).then((resp) => resp.json()).then((registerationOptionsData) => {
    startRegistration(registerationOptionsData).then((registerationResponseJson) => {
      ...
      }).then((resp) => resp.json()).then((verifyData) => {
        if (verifyData.verified) {
          return window.location.reload();
        }
        new FilamentNotification().title("Error verifying passkey").danger().send();
      }).catch(() => {
        new FilamentNotification().title("Error sending verification request").danger().send();
      });
    }).catch((err) => {
      console.log(err);
      new FilamentNotification().title("Error registering passkey").danger().send();
    });
  });
};
